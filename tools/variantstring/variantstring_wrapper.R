...
if (op == "check_variant_string") {
    result <- sapply(variant_strings, function(s) {
        tryCatch({ check_variant_string(s); TRUE }, error = function(e) FALSE)
    })
    ...
    result <- sapply(variant_strings, function(s) {
        tryCatch({ check_position_string(s); TRUE }, error = function(e) FALSE)
    })
    ...
    out <- data.frame(variant_string = variant_strings,
    ...
    out <- data.frame(variant_string = variant_strings, no_read_counts = result, stringsAsFactors = FALSE)
    write.table(out, output_file, sep = "\t", row.names = FALSE, quote = FALSE)
